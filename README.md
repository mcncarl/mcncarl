# Yichen / 逸尘

**AI product builder, open-source maintainer, and AI content creator.**

I build practical Agent systems, memory infrastructure, and AI workflows that turn research into usable products. I also share hands-on insights about AI products, tools, and real-world experiments on [X (@gengdaJ)](https://x.com/gengdaJ).

我是逸尘，一名 AI 产品构建者、开源维护者与 AI 自媒体博主。我关注 Agent 系统、记忆基础设施和实用 AI 工作流，也在 [X（@gengdaJ）](https://x.com/gengdaJ) 持续分享 AI 产品、工具与真实实践。

## Selected open-source work

- [yichen-skills](https://github.com/mcncarl/yichen-skills) — A collection of practical Codex and Agent Skills I build and maintain.
- [Agent Memory Vault](https://github.com/mcncarl/agent-memory-vault) — A Markdown-first shared memory system for Claude Code and Codex.
- [Ailu](https://github.com/mcncarl/ailu) — A local Agent chat and content publishing studio for Obsidian.
- [WeChat Relay](https://github.com/mcncarl/wechat-relay) — A self-hosted relay for WeChat Official Account draft APIs.

## Focus

- Agent systems and practical AI workflows
- Long-term memory infrastructure for AI agents
- AI product building and creator workflows
- Legal tech and evidence-led research

## Find me

- [Personal website — yichen.homes](https://yichen.homes)
- [X — @gengdaJ](https://x.com/gengdaJ)
